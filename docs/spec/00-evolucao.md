...
- **Entrevista de complexidade (respostas do usuário):** incômodos = arquitetura (2 repos/motor/CI) + excesso de camadas no site; essenciais = Caixa+CCA e IBIS+Dung; alvo = plataforma genérica; público inicial = humanos técnicos + agentes.
- **Resolução (2026-06-14):** o **protocolo autossuficiente foi implementado** (ver entrada seguinte). A **simplificação foi avaliada e descartada pelo usuário** — "está bom do jeito que está". Camadas e arquitetura (2 repos) **mantidas**.
...
### 2026-06-14/15 — Diversidade real (Codex + big-pickle) e resolução de conflitos
- **Contribuições de famílias de modelo diferentes chegaram de fato** no caso CIn: `agent:codex` (gpt-5) trouxe o parâmetro `provisao` (local/apuana/cloud_burst), critérios prazo/elasticidade, energia típica e removeu financiamento (ciclos 003-005, mergeado); `agent:opencode` (big-pickle) abriu PR com revisão diversa.
- **Conflito resolvido (review + reconciliação do PR #2):** o PR do big-pickle conflitava com o main (ambos mexiam em provisão). Reconciliado **sem duplicar**: mantida a provisão do Codex + adicionada só a opção nova `nuvem`; novo parâmetro `operacoes` (staffing); critério+métrica `opex_anual` (com `requires` corrigido) e métrica `tempo_producao`; argumentos re-apontados às opções reais e aresta de Dung indevida removida; ciclo renumerado 003→006. Autoria do big-pickle preservada. PR ficou *mergeable*, CI verde. Caixa do CIn: **12 params · 2.099.520 totais · 933.120 viáveis**.
- **Motor:** `ENUM_LIMIT` 2M→5M e `countViable` reescrito com poda incremental (push/pop, checagem O(1)) para suportar caixas grandes sem travar a interação.

## Estado atual (snapshot)
- **Fase:** chamada pública **recebendo contribuições reais de múltiplos agentes** (claude-opus-4-8, gpt-5/Codex, big-pickle); **quatro camadas ativas** + motor agnóstico de domínio. Plataforma e caso CIn em **repositórios separados**, ambos com site vivo publicado e **AGENTS.md autossuficiente** (protocolo embutido). Arquitetura/camadas **mantidas como estão** (simplificação descartada).
